                                        ...
                                        , m_chatModel(new QStandardItemModel(this)) // create the model to hold the messages
{
    ...
    ui->textLabel->setText("Aguardando conexão");
}
...
void ChatWindow::connectedToServer() {
    ...
    const QString newCanal = QInputDialog::getText(this, tr("Chose Canal that it is not zero"), tr("Canal"));
    if (newUsername.isEmpty() || newCanal.isEmpty()){
    ...
    attemptLogin(newUsername, newCanal);
}

void ChatWindow::attemptLogin(const QString &userName, const QString &newCanal) {
    // use the client to attempt a log in with the given username
    m_chatClient->login(userName, newCanal);
}

void ChatWindow::loggedIn(QString m_canal) {
    ...
    ui->textLabel->setText("Canal: " + m_canal);
}
