...
ChatClient::ChatClient(QObject *parent) : QObject(parent)
    ...
    connect(m_clientSocket, &QIODevice::readyRead, this, &ChatClient::onReadyRead);
...
void ChatClient::login(const QString &userName, const QString &newCanal) {
...
void ChatClient::jsonReceived(const QJsonObject &docObj) {
    ...
    if (typeVal.toString().compare(QLatin1String("login"), Qt::CaseInsensitive) == 0) { //It's a login message
        ...
        if (loginSuccess) {
            const QJsonValue resultCanal = docObj.value(QLatin1String("canal"));
            // we logged in successfully and we notify it via the loggedIn signal
            emit loggedIn(resultCanal.toString());
            return;
        }
